...
- 注册常驻的历史洞察回填任务 `com.fermiwang.feishu-archive-insights-backfill`
  （KeepAlive；依据 vMLX 引擎空闲状态从最早日期向最近日期连续回填）；
...
历史回填由独立 LaunchAgent 自主运行，不依赖 Codex 会话。它从本地三条档案中实际观察到的
