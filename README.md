...
回填计划全天候每 60 秒唤醒一次，不再设置 06:00–22:00 运行窗口或跨午夜截止点。提交前连续两次读取 11435
`/health`，要求模型精确匹配、`scheduler.num_running=0`、`num_waiting=0`，并且模型至少空闲
60 秒；每个 Map/Reduce 请求前还会再次检查调度器，遇到短时繁忙最多等待 15 分钟。健康
...
单次计划回填最多运行 30 分钟，到点后下一轮从分片断点继续。直连 11435 的常规日报也在每个 Map/Reduce 前执行相同引擎门禁。回填日志以及
