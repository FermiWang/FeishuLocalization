...
当前版本为 `0.3.6`，完整的“授权、同步、自动增量更新”链路仍以 macOS 为目标平台。Linux 和 Windows 可以复用部分源码，但不能在不改代码的情况下获得与 macOS 相同的完整同步能力。
...
没有最近 30 天限制。每次进程最多完成一个日期，状态原子保存在
...
回填计划每 30 分钟唤醒一次，只在 `Europe/Amsterdam` 的 06:00–22:00 启动新日期，给
03:30、03:45、04:00 三条同步和 04:30 日报留出优先窗口。提交前连续两次读取 11435
`/health`，要求模型精确匹配、`scheduler.num_running=0`、`num_waiting=0`，并且模型至少空闲
5 分钟；每个 Map/Reduce 请求前还会再次检查调度器，遇到短时繁忙最多等待 15 分钟。健康
字段缺失、模型不匹配或探测失败时本轮门禁保持关闭。vMLX 冷启动时若唯一缺失字段是尚未初始化的
`last_request_time`，系统会在确认模型匹配且队列为空后发起一次不含档案内容的最小预热请求，随后
按空闲冷却门槛延期；下一轮可自主恢复，不依赖人工或 Codex 先调用模型。未完整生成的历史日报不发布，
下一轮从分片断点继续。直连 11435 的常规日报也在每个 Map/Reduce 前执行相同引擎门禁。回填日志以及
