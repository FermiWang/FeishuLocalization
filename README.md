...
当前版本为 `0.6.1`，完整的“授权、同步、自动增量更新”链路仍以 macOS 为目标平台。Linux 和 Windows 可以复用部分源码，但不能在不改代码的情况下获得与 macOS 相同的完整同步能力。
...
- 以保留提交历史的 `meeting-minutes/` 独立运行单元整理详细会议记录；支持多录音、TXT/DOCX、粘贴文本、不可变修订和同源 DOCX。
- 使用独立 `meeting-records.sqlite3` 每 5 分钟从 179 通过固定 SSH 命令增量拉取已完成记录；不复制原始录音或识别稿。
- macOS 每天 03:30 增量同步消息、03:45 增量同步知识库、04:00 增量同步邮箱，并每 5 分钟同步详细会议记录；阅读器也提供手工同步按钮。
...
- 注册每 300 秒运行的会议记录同步任务 `com.fermiwang.feishu-archive-meeting-records-sync`；
...
此模式会额外注册每天 04:30 的洞察任务（05:00、05:30 幂等重试），以及依据模型空闲状态连续推进的 KeepAlive 历史回填任务。安装器不带选项时，新安装默认等同 `--without-insights`；已有安装则保留当前是否已经安装 Insights。正式操作建议始终写明选项，避免误解。
...
```bash
# 手工增量同步与状态检查
./bin/feishu-archive meeting-records-sync
./bin/feishu-archive meeting-records-status

# 安装后每 300 秒自动同步
launchctl print "gui/$(id -u)/com.fermiwang.feishu-archive-meeting-records-sync"
```

每日洞察运行前还会做一次最长 45 秒的会议同步。每个章节形成 `meeting:<id>:r<revision>:<section>` 证据；超长章节分块而不截断，并按会议填写日期归入自然日。某日尚无成功日报时，最新会议记录随首次生成自动纳入；某日已有日报后，会议新增、修订或删除只把该日期标为“会议证据已更新，待人工刷新”，不会自动改写旧日报。

阅读器的人工刷新按钮沿用邮箱本机会话，并要求回环 Host/Origin、`X-Feishu-Archive-Action: insights-refresh` 确认头和独占 `insights.lock`。刷新成功会生成引用新修订的新运行，旧运行保留；模型繁忙、失败或未发布时待刷新状态不清除。会议同步失败只让日报覆盖状态显示“不完整”，不会阻断聊天、知识库和邮箱阅读。

### 8. 每日洞察与本地 vMLX

每日洞察是聊天、知识库、邮箱和详细会议记录四条证据通道之上的只读派生层，保存到独立的
`insights.sqlite3`，不会自动发送消息、创建飞书任务或修改知识库。默认以
`Asia/Shanghai` 自然日生成昨日小结、今日规划和商业机会识别；页面入口为
...
系统降级为确定性覆盖统计，不推断新待办或商业机会，且不会影响四条证据通道。
如果邮件同步唯一的不完整项是磁盘容量门禁暂停了附件二进制下载，洞察仍可使用已同步的
邮件正文和附件元数据；其他未知或混合的 `partial` 状态仍会阻止报告发布。
降级或任一源通道覆盖不完整的运行记为 `partial`，不会替换上一份已发布日报，也不会
更新任务或机会台账。

历史回填由独立 LaunchAgent 自主运行，不依赖 Codex 会话。它从本地四条证据通道中实际观察到的
最早日期开始，按自然日严格向最近日期推进，目标上界始终包含“昨日”。只有源快照、模型、
提示词、分片参数和投影版本全部精确匹配的已发布常规日报，才会被认定为该日期已覆盖；存在
任意旧日报不会缩短范围。上界随后增长时只在末端追加日期，不会把正序游标重置到最早日。
没有最近 30 天限制。每一步最多完成一个日期，状态原子保存在
`insights/backfill-state.json`，模型繁忙、网络失败、系统重启或进程中断后仍从同一日期继续，
不会跨过失败日期形成静默空洞。04:30 常规日报若因负荷失败，会在 05:00、05:30 使用独立
断点重试；仍未成功的日期最终由回填通道按正序补齐。成功的 Map 分片以同一源快照和配置为键保存在
`insights/backfill-checkpoints`；重试只补未成功分片，整日报告发布后即清除对应断点。历史报告
单独导出到 `insights/exports/history`，永不替换当前活动日报，也不把当前的累计待办倒灌进旧
日期报告。

回填任务以 KeepAlive 常驻进程全天候运行，不再按固定间隔定时唤醒，也不设置 06:00–22:00 运行窗口或跨午夜截止点。
它以 `insights-backfill-step --loop` 循环执行：只要 192.168.100.179 上 11435 引擎空闲，就从最早日期向最近日期
连续执行回填步骤；引擎繁忙时每 30 秒轮询一次，等待空闲后继续。每个日期步骤提交前连续两次读取 11435
`/health`，要求模型精确匹配、`scheduler.num_running=0`、`num_waiting=0`；进程启动后或观察到外部引擎活动后的
首次提交要求模型至少空闲 60 秒，而由上一步自身占用引擎后的连续步骤只要求 10 秒空闲沉降，空闲即可接续执行。
每个 Map/Reduce 请求前还会再次检查调度器，遇到短时繁忙最多等待 15 分钟。健康
字段缺失、模型不匹配或探测失败时门禁保持关闭并轮询等待。vMLX 冷启动时若唯一缺失字段是尚未初始化的
`last_request_time`，系统会在确认模型匹配且队列为空后发起一次不含档案内容的最小预热请求，随后
按空闲冷却门槛延期；下一轮可自主恢复，不依赖人工或 Codex 先调用模型。未完整生成的历史日报不发布，
单次回填步骤最多运行 30 分钟，到点后下一步从分片断点继续；每一步独立获取和释放 `insights.lock`，不会长期
占用而阻塞 04:30 常规日报。连续 10 步出错时循环进程主动退出，由 launchd 的 ThrottleInterval 延迟后自动重启。
常驻回填使用独立的本地隧道端口 18136，不会与常规日报的 18135 隧道冲突。
直连 11435 的常规日报也在每个 Map/Reduce 前执行相同引擎门禁。回填日志以及
计划任务的常规日报日志只写日期、状态和错误码，不写报告或邮件派生正文。

当前 8067 代理没有不含正文的队列摘要端点，且本机尚未配置其 Bearer；系统不会读取会返回
`question` 正文的 `/api/queue`。因此负荷门禁采用 11435 引擎的连续稳定采样，降低与代理任务
竞争的概率，但不把它表述为跨 8067/11435 的原子队列预约。

可手工查看状态或触发一个受同样门禁约束的步骤：
...
历史分析明确采用“当前本地归档快照”口径，不声明飞书服务端历史绝对完整。邮件文件夹归属、
聊天撤回状态按当前归档状态解释；如果知识库当前正文版本晚于目标日期，该历史日只发送文档
元数据，不把未来版本正文交给模型。任务与机会按最早到最近的事件顺序逐步投影；每个邮件线程、
聊天话题/会话或知识库文档容器独立生成投影，同一条跨容器观察会分别更新各容器，不凭标题跨容器
合并或关闭事项，避免不同来源的同名事项相互影响。升级前的机器投影
保留审计记录但退出当前台账，人工维护的状态不清除。可由来源范围、标题、项目和负责人唯一证明
为同一事项的旧人工状态继续作为投影锚点，不会被重新打开；旧的多容器人工项保留为一个标记
`requires_split` 的 legacy 锚点，等待人工拆分而不复制状态，存在多个候选时不猜测。游标到达最近日期后，系统再从最早日期到
最近日期复核一轮每日源快照，复核通过才标记累计台账完成；之后继续循环复核。若后来补入或修改
已处理日期的历史证据，会建立新 campaign、保留变更记录，并从最早日期重新正序投影。
...
邮箱 API 默认在未建立本机会话时返回 `401`。使用下列命令生成并打开带 URL fragment 的解锁地址；fragment 不会作为 HTTP 请求的一部分发送给服务端，页面只用它交换 15 分钟、仅驻留内存的本机会话：
...
如需让当前档案在服务重启和重新部署后仍保持本机解锁，可显式启用永久模式：

```bash
./bin/feishu-archive mail-reader-url --permanent --open
```

该命令在档案根目录写入权限为 `0600` 的版本化策略文件 `mail-reader.always-unlocked`，并通过同权限的 `mail-reader.policy-generation` 撤销旧会话，不会把长期密钥放入 URL 或 Cookie。阅读器仍只监听回环地址，并严格拒绝非字面回环 Host、端口不匹配及非本机 Origin。需要恢复默认锁定时执行：

```bash
./bin/feishu-archive mail-reader-url --lock
```

永久模式会允许同机进程、浏览器扩展和能够连接本机回环端口的其他本机用户直接读取邮件；它不防主动配置的代理、隧道或端口转发。只应在受信任、启用磁盘加密且账户访问受控的 Mac 上使用。
...
- 阅读器只允许绑定 `127.0.0.1` 或 `localhost`，并校验请求 Host 与实际监听端口。
- OAuth `state` 会校验，授权码只在本地回调服务中交换。
- macOS 上的应用凭据和令牌保存在 Keychain，日志和 SQLite 不写入这些值。
- 邮箱 API 默认需要短期、仅驻留服务进程内存的本机会话；解锁密钥通过 URL fragment 交给页面，不写入查询字符串或服务访问日志。永久本机解锁必须显式启用，并可随时回锁。
...
| 直接访问 `/api/mail/*` 返回 `401` | 默认模式要求短期本机会话；运行 `mail-reader-url --open` 临时解锁。若确认接受同机访问风险，可运行 `mail-reader-url --permanent --open` 持久解锁；用 `mail-reader-url --lock` 恢复默认锁定。不要把 fragment 中的密钥复制到日志或聊天中。 |
