...
“整理详细会议记录”只允许调用 179 本机的精确模型 ID `vmlx/qwen3.8-27b-8bit`。每个模型阶段都会检查 `/health`、`scheduler.num_running`、`scheduler.num_waiting` 和 `/v1/models`；模型不匹配、健康字段缺失或返回别名时关闭处理，模型繁忙时任务回到单任务队列等待，不用其他模型替代。
...
| 变量 | 默认值 | 说明 |
| --- | --- | --- |
| `LLM_BASE_URL` | `http://127.0.0.1:11435/v1` | 179 本机 vMLX 端点 |
| `LLM_MODEL` | `vmlx/qwen3.8-27b-8bit` | 只用于一致性核验，其他值会被拒绝 |
